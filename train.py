...
def main_worker(gpu, save_dir, ngpus_per_node, args):
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for bidx, data in enumerate(train_loader):
            ...
            out = model(inputs, optimizer, step, None)
        ...
        if args.use_sphere_dist:
            model.update_var(args.step_var, args.stop_var)

        # evaluate on the validation set
        if not args.no_validation and (epoch + 1) % args.val_freq == 0:
            from utils import validate
            validate(test_loader, model, epoch, None, save_dir, args, clf_loaders=clf_loaders)
        ...
        if (epoch + 1) % args.viz_freq == 0:
            ...
            if args.use_latent_flow:
                ...
                _, samples = model.sample(num_samples, num_points, gpu=args.gpu)
                results = []
                for idx in range(num_samples):
                    ...
                    results.append(res)
                res = np.concatenate(results, axis=1)
                imageio.imsave(os.path.join(save_dir, 'images', 'tr_vis_sampled_epoch%d-gpu%s.png' % (epoch, args.gpu)),
                                  res.transpose((1, 2, 0)))
