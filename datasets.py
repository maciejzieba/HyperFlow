...
class Uniform15KPC(Dataset):
    def __init__(self, root_dir, subdirs, tr_sample_size=10000,
        ...
        self.all_points = []
        if self.split == 'all':
            self.get_necessary_data(root_dir, 'train')
            self.get_necessary_data(root_dir, 'test')
            self.get_necessary_data(root_dir, 'val')
        else:
            self.get_necessary_data(root_dir, self.split)
    ...
    def get_necessary_data(self, root_dir, split):
        for cate_idx, subd in enumerate(self.subdirs):
            ...
            for x in os.listdir(sub_path):
                if not x.endswith('.npy'):
                    continue
                all_mids.append(os.path.join(split, x[:-len('.npy')]))
...
class ShapeNet15kPointClouds(Uniform15KPC):
    def __init__(self, root_dir="data/ShapeNetCore.v2.PC15k",
        ...
        self.root_dir = root_dir
        self.split = split
        assert self.split in ['train', 'test', 'val', 'all']
...
def get_datasets(args):
    if args.dataset_type == 'shapenet15k':
        ...
        tr_dataset = ShapeNet15kPointClouds(
            categories=args.cates, split=train_tag,
            tr_sample_size=args.tr_max_sample_points,
            te_sample_size=args.te_max_sample_points,
